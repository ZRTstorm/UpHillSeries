...
public interface RouteGroupRepository  extends JpaRepository<RouteGroup, Long> {
    ...
    @Modifying
    @Query("delete from RouteGroup r where r.route1.id = :routeId or r.route2.id = :routeId")
    ...
    @Query("select case when r.route1.id = :routeId then r.route2.id else r.route1.id end " +
            "from RouteGroup r " +
            "where r.route1.id = :routeId or r.route2.id = :routeId")
